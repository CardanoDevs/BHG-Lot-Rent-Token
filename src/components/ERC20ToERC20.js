...
import { Button, Form, Row, Col , InputGroup,FormControl, Modal} from 'react-bootstrap';
            ...
            <div>
                ...
                <Form>
                    ...
                    <InputGroup className="mb-3">
                        <InputGroup.Text>TKN</InputGroup.Text>
                        <InputGroup.Text>Open Token Value</InputGroup.Text>
                        <FormControl aria-label="Dollar amount (with dot and two decimal places)" type="text" placeholder="open Value" value={this.props.openValue} onChange={this.handleOpenValue} defaultValue={this.state.openValue}/>
                    </InputGroup>
                    <InputGroup>
                        <FormControl aria-label="Dollar amount (with dot and two decimal places)" type="text" placeholder="Close Value" value={this.props.closeValue} onChange={this.handleCloseValue} defaultValue={this.state.closeValue}/>
                        <InputGroup.Text>Close Token Value </InputGroup.Text>
                        <InputGroup.Text>TKN</InputGroup.Text>
                    </InputGroup>
                    <Form.Group className="mb-3">
                        <Form.Label>Swap Process</Form.Label>
                        <Form.Control as="textarea" rows={7} value={this.state.process} readOnly={true} />
                    </Form.Group>
                    <Form.Group>
                        <Button variant="primary" onClick={() => this.open()} >Open</Button>
                        <Button variant="success" className="mb-right" onClick={() => this.close()} >Close</Button>
                        <Button variant="danger" className="mb-right" onClick={() => this.expire()} >Expiry</Button>
                    </Form.Group>
                </Form>
            </div>
        );
